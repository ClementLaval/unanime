import { getNumberOfAnimations, retrieveAnimationIndex } from "./utils.js";

export function onFinish(action, index, animationsArray){
  if(typeof index === 'number'){
    const {mainIndex, secondIndex} =  retrieveAnimationIndex(index, animationsArray);
    return animationsArray[mainIndex][secondIndex].onfinish = () => {
      action();
    }
  } 
  let count = 0;
